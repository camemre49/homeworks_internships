...
int main()
{
    ...
    printAndDestroy(concat(take(1, repeat(4.f)) , take(1, repeat(2.f))));

    printAndDestroy(pair(take(5, count(1.f, 1.f))));
    
    printAndDestroy(take(5, min(count(4.f, 1.f),count(1.f, 3.f))));

    printAndDestroy(select(count(10.f, 10.f), concat(take(3, count(1u, 2u)), take(3, count(2u, 2u)))));

    printAndDestroy(skip(5, take(3,count(1,3))));
